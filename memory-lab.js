...
function dual(){begin((p,t)=>{const n=clamp(Math.floor(p.count/2),2,4),a=drawUnique(ICONS,n,`dual-a:${S.age}`),b=drawUnique(ICONS.filter(x=>!a.includes(x)),n,`dual-b:${S.age}`),seq=[];for(let i=0;i<n;i++)seq.push(a[i],b[i]);const studyMs=clamp(11000+n*1800,12800,17500);stage(shell('dual',`<div class="memory-instruction-banner"><strong>Memorize now.</strong> Study both streams carefully. The board will stay visible until the study timer finishes.</div><div class="dual-streams"><div><small>STREAM A</small><div>${a.join(' ')}</div></div><div><small>STREAM B</small><div>${b.join(' ')}</div></div></div><div class="memory-progress dual-study-progress" id="dual-study-progress">Study time: ${(studyMs/1000).toFixed(1)} sec</div>`));let remaining=studyMs;const update=()=>{if(t!==S.memoryRoundToken)return;remaining=Math.max(0,remaining-100);const el=$('dual-study-progress');if(el)el.textContent=remaining>0?`Memorize now • ${(remaining/1000).toFixed(1)} sec remaining`:'Get ready…';if(remaining>0)S.studyTicker=setTimeout(update,100)};S.studyTicker=setTimeout(update,100);S.timer=setTimeout(()=>{if(t!==S.memoryRoundToken)return;clearTimeout(S.studyTicker);stage(shell('dual',`<div class="memory-instruction-banner"><strong>Now reproduce the pattern.</strong> Tap Stream A, then Stream B, and keep alternating until you finish.</div><div id="dual-options" class="memory-items"></div><div class="picked-sequence" id="dual-picked">Your sequence: <span>—</span></div>`));const box=$('dual-options'),picked=$('dual-picked').querySelector('span');let i=0;S.active=true;shuffle([...new Set(seq)]).forEach(x=>{const b=optionButton(x);b.onclick=()=>{if(!S.active)return;if(x!==seq[i]){b.classList.add('bad');fail();return}b.classList.add('selected');picked.textContent=(picked.textContent==='—'?'':picked.textContent+' ')+x;i++;if(i===seq.length)complete()};box.appendChild(b)});timeout(p.response)},studyMs)})}
