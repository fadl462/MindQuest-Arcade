...
function sequence(){begin((sp,t)=>{
 ...
 S.timer=setTimeout(()=>{if(t!==S.memoryRoundToken)return;$('game-stage').innerHTML=`<div class="memory-wrap">${header('sequence','Tap the objects in exactly the same order.')}<div id="sequence-choices" class="memory-items"></div><div id="sequence-picked" class="picked-sequence"></div></div>`;const box=$('sequence-choices'),picked=$('sequence-picked');let n=0;S.active=true;shuffle([...new Set(a)]).forEach(x=>{const b=document.createElement('button');b.className='choice';b.textContent=x;b.setAttribute('aria-pressed','false');b.onclick=()=>{if(!S.active)return;if(x!==a[n]){b.classList.add('bad');fail()}else{b.classList.add('good','selected');b.setAttribute('aria-pressed','true');b.dataset.picks=String((Number(b.dataset.picks)||0)+1);picked.textContent+=(n?' ':'')+x;if(++n===a.length)complete()}};box.appendChild(b)});deadline(sp.response)},sp.show)
})}
...
function order(){begin((sp,t)=>{
 ...
 S.timer=setTimeout(()=>{if(t!==S.memoryRoundToken)return;$('game-stage').innerHTML=`<div class="memory-wrap">${header('order','Rebuild the order from first to last.')}<div id="order-options" class="memory-items"></div><div id="order-picked" class="picked-sequence"></div></div>`;const box=$('order-options'),picked=$('order-picked');let n=0;S.active=true;shuffle([...new Set(a)]).forEach(x=>{const b=document.createElement('button');b.className='choice';b.textContent=x;b.setAttribute('aria-pressed','false');b.onclick=()=>{if(!S.active)return;if(x!==a[n]){b.classList.add('bad');fail()}else{b.classList.add('good','selected');b.setAttribute('aria-pressed','true');b.dataset.picks=String((Number(b.dataset.picks)||0)+1);picked.textContent+=(n?' → ':'')+x;if(++n===a.length)complete()}};box.appendChild(b)});deadline(sp.response)},sp.show)
})}
...
function reverse(){begin((sp,t)=>{
 ...
 S.timer=setTimeout(()=>{if(t!==S.memoryRoundToken)return;$('game-stage').innerHTML=`<div class="memory-wrap">${header('reverse','Tap the objects in reverse order.')}<div id="reverse-choices" class="memory-items"></div><div id="reverse-picked" class="picked-sequence"></div></div>`;const box=$('reverse-choices'),picked=$('reverse-picked');let n=0;S.active=true;shuffle([...new Set(seq)]).forEach(x=>{const b=document.createElement('button');b.className='choice';b.textContent=x;b.setAttribute('aria-pressed','false');b.onclick=()=>{if(!S.active)return;if(x!==answer[n]){b.classList.add('bad');fail()}else{b.classList.add('good','selected');b.setAttribute('aria-pressed','true');b.dataset.picks=String((Number(b.dataset.picks)||0)+1);picked.textContent+=(n?' ':'')+x;if(++n===answer.length)complete()}};box.appendChild(b)});deadline(sp.response)},sp.show)
})}
...
function direction(){begin((sp,t)=>{
 ...
 S.timer=setTimeout(()=>{if(t!==S.memoryRoundToken)return;$('game-stage').innerHTML=`<div class="memory-wrap">${header('direction','Repeat the direction sequence exactly.')}<div id="direction-choices" class="memory-items"></div><div id="direction-picked" class="picked-sequence"></div></div>`;const box=$('direction-choices'),picked=$('direction-picked');let n=0;S.active=true;shuffle(ARROWS.slice()).forEach(x=>{const b=document.createElement('button');b.className='choice';b.textContent=x;b.setAttribute('aria-pressed','false');b.onclick=()=>{if(!S.active)return;if(x!==seq[n]){b.classList.add('bad');fail()}else{b.classList.add('good','selected');b.setAttribute('aria-pressed','true');b.dataset.picks=String((Number(b.dataset.picks)||0)+1);picked.textContent+=(n?' ':'')+x;if(++n===seq.length)complete()}};box.appendChild(b)});deadline(sp.response)},sp.show)
})}
...
function interference(){begin((sp,t)=>{
 ...
 S.timer=setTimeout(()=>{if(t!==S.memoryRoundToken)return;$('game-stage').innerHTML=`<div class="memory-wrap">${header('interference','Tap the target objects in the original order.')}<div id="interference-choices" class="memory-items"></div><div id="interference-picked" class="picked-sequence"></div></div>`;const box=$('interference-choices'),picked=$('interference-picked');let n=0;S.active=true;shuffle([...new Set(target.concat(distract))]).forEach(x=>{const b=document.createElement('button');b.className='choice';b.textContent=x;b.setAttribute('aria-pressed','false');b.onclick=()=>{if(!S.active)return;if(x!==target[n]){b.classList.add('bad');fail()}else{b.classList.add('good','selected');b.setAttribute('aria-pressed','true');b.dataset.picks=String((Number(b.dataset.picks)||0)+1);picked.textContent+=(n?' ':'')+x;if(++n===target.length)complete()}};box.appendChild(b)});deadline(sp.response)},Math.max(1800,sp.show+900))
})}
