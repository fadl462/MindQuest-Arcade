...
const saveAndExit=()=>{if(!MQ.state.game)return;saved[keyFor(MQ.state.age,MQ.state.game)]=checkpoint();try{localStorage.setItem(STORE,JSON.stringify(saved));localStorage.setItem(SESSION,JSON.stringify({age:MQ.state.age,game:MQ.state.game}));localStorage.setItem(AGE_KEY,String(MQ.state.age));localStorage.setItem(VIEW_KEY,'home');if(MQ.state.account)localStorage.setItem(ACCOUNT_KEY,JSON.stringify(MQ.state.account));localStorage.setItem(INTEL_KEY,JSON.stringify(intel))}catch{}arcadeExitSave=true;try{if(typeof MQ.goHome==='function')MQ.goHome();}finally{setTimeout(()=>arcadeExitSave=false,1500)}};
...
.mq-saved-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;padding:2px 0 6px}
.mq-saved-list .mq-checkpoint-card{min-width:0}
.mq-saved-list .mq-resume-card{min-height:64px}
