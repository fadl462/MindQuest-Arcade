...
async function resetAllTestData(){
  const ok=confirm('Reset MindQuest to a completely new-player test state? This will erase all local XP, levels, badges, streaks, practice history, accounts, checkpoints and saved sessions on this device.');
  if(!ok)return;
  const prefix=/^mindquest-/i;
  try{Object.keys(localStorage).forEach(k=>{if(prefix.test(k)||k==='mindquest-current-view-v4')localStorage.removeItem(k)})}catch{}
  ...
  try{window.location.replace('./index.html?reset='+Date.now())}catch{window.location.reload()}
}
...
window.addEventListener('beforeunload',()=>{savePlayer();if(document.querySelector('.screen.active')?.id==='game'){writeLiveSession({phase:readLiveSession()?.phase||'active'});saveCheckpoint()}});
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden'){savePlayer();if(document.querySelector('.screen.active')?.id==='game'){writeLiveSession({phase:readLiveSession()?.phase||'active'});saveCheckpoint()}}});
