...
function syncPlayerIdentity(){const a=MQ.state.account||{};const n=document.getElementById('player-identity-name');if(n)n.textContent=a.displayName||'Guest';const f=document.getElementById('player-identity-flag');if(f){const c=String(a.countryCode||'').toLowerCase();f.hidden=!c;f.title=a.country||'';f.setAttribute('aria-label',a.country||'Country');if(c){f.innerHTML=`<img src="https://flagcdn.com/24x18/${c}.png" alt="${a.country||c} flag" loading="eager" onerror="this.remove();this.parentElement.textContent='${c.toUpperCase()}'">`;}else f.innerHTML='';}syncExperienceMode();}
