...
const restore=p=>{Object.assign(MQ.state,p);MQ.state.active=false;MQ.state.timer=null;MQ.state.skills=p.skills||MQ.state.skills;MQ.state.skillStats=p.skillStats||MQ.state.skillStats;MQ.state.account=p.account||MQ.state.account;MQ.state.premium=!!p.premium;syncPlayerIdentity();MQ.updateGlobal()};
...
function ensureIntel(){intel.history=intel.history||[];intel.totalLevels=intel.totalLevels||0;intel.milestones=intel.milestones||[];intel.games=intel.games||{};intel.streakBest=intel.streakBest||0;intel.daily=intel.daily||{date:'',done:false};if(!intel.createdAt)intel.createdAt=new Date().toISOString();}
function dailyMission(){ensureIntel();const d=new Date().toISOString().slice(0,10);if(intel.daily.date!==d){const missions=[['Solve 3 challenges','Complete three activities carefully.'],['Build your streak','Complete two levels without a failed attempt.'],['Explore a skill','Play a game you have not played today.'],['Think before you tap','Complete one cognitive challenge without rushing.']];const m=missions[(new Date().getDate()+MQ.state.age)%missions.length];intel.daily={date:d,done:false,title:m[0],desc:m[1]};}return intel.daily}
...
function syncPlayerIdentity(){const n=document.getElementById('player-identity-name');if(n)n.textContent=MQ.state.account?.displayName||'Guest';}
