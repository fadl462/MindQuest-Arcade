...
async function resetAllTestData(){
  const ok=confirm('Reset all MindQuest test data on this device?\n\nThis will erase XP, levels, badges, streaks, retries, practice history, accounts, checkpoints and active sessions. MindQuest will return to a brand-new Level 1 state.');
  if(!ok)return;
  try{sessionStorage.clear()}catch{}
  try{
    const keys=[];
    for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k&&(/^mindquest-/i.test(k)||/^mq-/i.test(k)))keys.push(k)}
    keys.forEach(k=>{try{localStorage.removeItem(k)}catch{}});
    ['mindquest-progress-v3','mindquest-progress-v4','mindquest-player-v1','mindquest-profile-v1','mindquest-current-view-v4','mindquest-current-view-v5'].forEach(k=>{try{localStorage.removeItem(k)}catch{}});
  }catch{}
  try{if('indexedDB' in window&&indexedDB.databases){const dbs=await indexedDB.databases();await Promise.all((dbs||[]).filter(d=>d&&d.name).map(d=>new Promise(resolve=>{try{const r=indexedDB.deleteDatabase(d.name);r.onsuccess=r.onerror=r.onblocked=()=>resolve()}catch{resolve()}})))}}catch{}
...
function syncPlayerIdentity(){const a=MQ.state.account||{};const n=document.getElementById('player-identity-name');if(n)n.textContent=a.displayName||'Guest';const f=document.getElementById('player-identity-flag');if(f){const c=String(a.countryCode||'').trim().toLowerCase();if(c){f.textContent='';f.hidden=false;f.dataset.country=c.toUpperCase();f.title=a.country||c.toUpperCase();f.setAttribute('aria-label',a.country?`Country: ${a.country}`:`Country: ${c.toUpperCase()}`);f.style.backgroundImage=`url(https://flagcdn.com/w40/${encodeURIComponent(c)}.png)`;}else{f.textContent='';f.hidden=true;f.style.backgroundImage='none';delete f.dataset.country;}}syncExperienceMode();}
...
function restoreSavedView(){
  ...
  if(session&&session.view==='game'&&session.game){
    ...
    if(meta){const icon=document.getElementById('game-icon'),name=document.getElementById('game-name'),skill=document.getElementById('game-skill');if(icon)icon.textContent=meta[0];if(name)name.textContent=meta[1];if(skill)skill.textContent=meta[2]}
