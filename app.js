...
function buildReport(level){
  const s=state.skillStats||{};
  if(state.game==='memory'){
    const completed=clamp(Number(level||1),1,20),activities=completed*4;
    const stage=completed>=20?{
      title:'You completed the full Memory Lab journey.',
      summary:'Across 20 levels and 80 activities, you have practised remembering, focusing, ordering, locating, comparing and working with information under increasing challenge.',
      built:['Sustained recall — keeping information in mind long enough to use it.','Focused attention — staying with the task while details or distractions increase.','Flexible memory — remembering order, locations, features, quantities and changes.','Memory control — holding, updating and using information rather than only recognising it.']
    }:completed>=15?{
      title:'You have moved from remembering information to working with it.',
      summary:`Your progress now includes the first ${completed} levels (${activities} completed activities). You have built a broader memory toolkit and are handling more complex combinations of information.`,
      built:['Recall & recognition — retrieving information after it disappears.','Order & location — remembering where things were and the sequence they appeared in.','Detail & comparison — noticing features, quantities and meaningful changes.','Mental control — holding information while following an additional rule or instruction.']
    }:completed>=10?{
      title:'Your memory toolkit is becoming broader and more reliable.',
      summary:`You have completed ${completed} levels and ${activities} Memory Lab activities. The early foundation is now being extended into more detailed, multi-step memory work.`,
      built:['Visual recall — remembering objects and selecting the ones you saw.','Attention control — concentrating on the relevant information before responding.','Sequence & order — keeping information in the correct order.','Spatial & detail memory — remembering positions and important features.']
    }:{
      title:'You have built the foundation for stronger memory and attention.',
      summary:`You have completed ${completed} levels and ${activities} Memory Lab activities. This first checkpoint reflects the skills you have practised so far — not a test score or intelligence rating.`,
      built:['Recall — remembering information after it disappears.','Focused attention — looking carefully and staying with the task.','Recognition — identifying information you have seen before.','Following memory rules — responding according to the instruction rather than guessing.']
    };
    const streak=Number(state.streak||0);
    return `<div class="memory-report">
      <div class="memory-report-summary"><div><span class="memory-report-kicker">YOUR MEMORY LAB JOURNEY</span><h2>${stage.title}</h2><p>${stage.summary}</p></div><div class="memory-report-stats"><div><b>${completed}</b><span>Levels completed</span></div><div><b>${activities}</b><span>Activities completed</span></div><div><b>${streak}</b><span>Current streak</span></div></div></div>
      <div class="memory-built-grid">${stage.built.map((x,i)=>{const parts=x.split(' — ');return `<article><span>${['🧠','🎯','🧩','🔄'][i]}</span><div><strong>${parts[0]}</strong><p>${parts[1]}</p></div></article>`}).join('')}</div>
      <div class="memory-cumulative-note"><strong>How to read this checkpoint</strong><p>This is a cumulative progress report. It describes what you have practised across every completed Memory Lab level up to Level ${completed}. It is not a percentage, grade, IQ score or clinical assessment.</p></div>
    </div>`;
  }
  const items=[['🧠 Memory & attention',s.memory+s.attention,'Remembering information, staying focused and handling more than one piece of information at a time.'],['🔎 Logic & problem solving',s.logic+s.problemSolving,'Looking for patterns, testing ideas and working through a challenge before choosing an answer.'],['⚡ Reaction & precision',s.reaction+s.precision,'Responding at the right moment and coordinating what you see with what you do.'],['🤝 Social & emotional skills',s.teamwork+s.empathy+s.responsibility,'Practising cooperation, perspective-taking, patience and responsible choices.'],['🌍 Knowledge & financial thinking',s.geography+s.financialLiteracy,'Using real-world knowledge, quantities and practical information to make sensible decisions.']];
  return `<div class="performance-grid">${items.map(x=>`<article class="performance-card"><div class="performance-top"><strong>${x[0]}</strong><span>${x[1]} practice points</span></div><p>${x[2]}</p><div class="performance-bar"><span style="width:${clamp(x[1]*6,4,100)}%"></span></div></article>`).join('')}</div><div class="performance-note"><strong>What this means</strong><p>These practice points are indicators of the skills exercised through gameplay. They are not a clinical, academic or intelligence assessment.</p></div>`;
}
